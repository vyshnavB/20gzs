    ...
    if request.method == "POST":
        ...
        try: 
            ...
            if profile is not None:
                ...
                user.profile_pic = "../static/pro.png"
            user.cover = cover           
            user.save()
...
def profile(request, username):
    ...
    if request.user.is_authenticated:
        followings = Follower.objects.filter(followers=request.user).values_list('user', flat=True)
        suggestions = User.objects.filter(pk__in=followings).exclude(username=request.user.username).order_by("?")[:6]
...
def user_create_post(request):
    if request.method == 'POST':
        ...
        try:
            post = Post.objects.create(creater=request.user, content_text=text, content_image=pic,status=status,Product_Price=Product_Price,posts_type="user_post",doc=doc,vedio=vedio)
...
def create_pagepost(request,pageid):
    ...
    if request.method == 'POST':
        ...
        post = Post(creater=request.user,page_id=pag,content_text=content_text, content_image=content_image,page_name=page_name,status=status,Product_Price=Product_Price,posts_type="page_post",vedio=vedio,doc=doc)
        post.save()
...
def notification(request,nid):
    ...
    return render(request,"notification.html",{'fr':fr,"inv":inv,'pag':pag})  
